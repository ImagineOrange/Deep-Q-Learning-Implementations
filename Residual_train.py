...
from Dynamic_Residual import Dynamic_Residual
...
if __name__ == "__main__":
    ...
    online_DDQN = Dynamic_Residual(input_shape = [frame_stack,Environment.width+1,Environment.height+1],
                                n_actions = 4,
                                epsilon_start = epsilon_start,
                                epsilon_min = epsilon_min,
                                d_hidden=1024,
    ...
    target_DDQN = Dynamic_Residual(input_shape = [frame_stack,Environment.width+1,
                                        Environment.height+1],
                                        n_actions = 4,
                                        epsilon_start = epsilon_start,
                                        epsilon_min = epsilon_min,
    ...
    while not stop_training:     
        ...
        #all actions after intro state-filling
            ...
            #if not first action, current_dir = last action
        ...
        if frame % 10_000 == 0:
            ...
            with torch.no_grad():
                running_qs_min.append((online_DDQN.forward(state_tensor)).cpu().detach().numpy().min())
                running_qs_max.append((online_DDQN.forward(state_tensor)).cpu().detach().numpy().max())
        ...
        if frame % update_every == 0 and frame > training_after:
            ...
            #clip gradients
            torch.nn.utils.clip_grad_norm_(online_DDQN.parameters(), max_norm=1.0)
